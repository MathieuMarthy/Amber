...
void main() {
  ...
  group('SubscriptionRepository - getByMonth', () {
    test('retrieves subscription that starts in the target month', () async {
      ...
      final result = await repository.getByMonth(DateTime(2026, 6, 1));
      expect(result.length, 1);
      expect(result.first.id, sub.id);
    });

    test('retrieves subscription that starts before and repeats indefinitely', () async {
      final sub = await repository.create(
        name: 'Spotify',
        price: 999,
    ...
    test('does not retrieve subscription that ended before the target month via repeatUntil', () async {
      await repository.create(
        name: 'Ended Sub',
        price: 1000,
        startDay: DateTime(2026, 1, 1),
    ...
    test('retrieves subscription that ends during the target month via repeatUntil', () async {
      final sub = await repository.create(
        name: 'Ending Sub',
        price: 1000,
        startDay: DateTime(2026, 1, 1),
    ...
    test('does not retrieve subscription that ended before target month via repeatXTimes', () async {
    ...
    test('retrieves subscription active during target month via repeatXTimes', () async {
